...
function ComboboxInput({
   ...
   showTrigger?: boolean;
   showClear?: boolean;
   triggerClassName?: React.ComponentProps<typeof ComboboxTrigger>['className'];
}) {
   return (
         ...
         <InputGroupAddon align="inline-end">
            {showTrigger && (
               <InputGroupButton
                  ...
                  disabled={disabled}
               >
                  <ComboboxTrigger className={cn(triggerClassName)} />
               </InputGroupButton>
            )}
...
function ComboboxItem({
   ...
   return (
      <ComboboxPrimitive.Item
         ...
         className={cn(
            "relative flex w-full cursor-default items-center gap-2 rounded-sm py-1.5 pr-8 pl-2 text-sm outline-hidden select-none dark:data-highlighted:bg-accent data-highlighted:bg-muted-foreground/35 data-highlighted:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
            className,
         )}
         {...props}
      >
